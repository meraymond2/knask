...
namespace parser {
    ...
    struct ParseArgs {
        ...
        ParseArgs(std::vector<Spec> specs, std::vector<std::byte> bytes) : specs(std::move(specs)),
    ...
    struct PrintParsed {
        void operator()(uint8_t &val) { std::cout << "u8: " << static_cast<uint16_t>(val) << std::endl; }
        ...
        void operator()(int8_t &val) { std::cout << "i8: " << static_cast<int16_t>(val) << std::endl; }
        ...
        void operator()(std::string &val) { std::cout << "str: " << val << std::endl; }
    };
}
